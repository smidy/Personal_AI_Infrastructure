...
`~/.claude/PAI/USER/SKILLCUSTOMIZATIONS/QuestForge/`
...
- **Read Telos** via `bun Tools/ReadTelos.ts` (wraps the Telos `getTelosContext()` pattern over
  `~/.claude/PAI/USER/TELOS/`). Read live every run — goals/problems change.
...
```bash
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","skill":"QuestForge","workflow":"WORKFLOW_USED","input":"8_WORD_SUMMARY","status":"ok|error","duration_s":SECONDS}' >> ~/.claude/PAI/MEMORY/SKILLS/execution.jsonl
```
